// kernels/vector_add/vector_add_v3_g&v.cu
...
// 3. CUDA kernel(s)
// =====================
__global__ void vector_add_v3_kernel(int *A,
                                     int *B,
                                     int *C,
                                     int N)
{
    ...
    {
        int tail_start = loop * 4;
        int i = tail_start + (idx - loop);
        C[i] = A[i] + B[i];
    }
}
...
int main(int argc, char **argv)
{
    ...
    // ---- 6.5 warm-up ----
    vector_add_v3_kernel<<<grid, block>>>(d_A, d_B, d_C, N);
